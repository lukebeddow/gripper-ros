...
def modify_xml_tag(filename, tagname, target_text):
  """
  ...
  # parse and extract the xml tree
  ...
  # finally, overwrite the given file with the new xml
  tree.write(filename, xml_declaration=True, encoding='utf-8')

# ----- Start by getting paths and the gripper.yaml configuration file ----- #
...
torsion = gripper_details["gripper_config"]["torsion"]
effort_control = gripper_details["gripper_config"]["effort_control"]

# ----- Next modify the controller files ----- #

# now open the other two controller base files, which we will add to
if effort_control:
  # open the controller information for the arm and hand, then save
  with open(control_path + "//config//arm_hand_effort_control.yaml") as file:
    base_control_dict = yaml.safe_load(file)
else:
  # open the controller information for the arm and hand, then save
  with open(control_path + "//config//arm_hand_position_control.yaml") as file:
    base_control_dict = yaml.safe_load(file)
...
# ----- Modify the xacro/urdf files ----- #

if effort_control:
  modify_xml_tag(description_path + "//xacro//panda.control.xacro",
                 "hardwareInterface", "hardware_interface/EffortJointInterface")
else:
  modify_xml_tag(description_path + "//xacro//panda.control.xacro",
                 "hardwareInterface", "hardware_interface/PositionJointInterface")

# ----- Finish by saving the configuration details in gripper.yaml ----- #

# finally, save the details of our updates in the gripper.yaml file
gripper_details["gripper_config"]["previous_update"]["old_is_segmented"] = is_segmented
gripper_details["gripper_config"]["previous_update"]["old_num_segments"] = num_segments
gripper_details["gripper_config"]["previous_update"]["torsion"] = torsion
gripper_details["gripper_config"]["previous_update"]["effort_control"] = effort_control
...
# exit the script so that roslaunch knows the process has finished
