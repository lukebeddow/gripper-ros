...
#include <gripper_control/Efforts.h>
...
public:
    ...
    bool init(std::vector<hardware_interface::JointHandle> &joint_handles, ros::NodeHandle &nh)
    {
        ...
        KDL::Chain chain;
        if (!tree.getChain("panda_link0", "panda_link8", chain)) {
        ...
        ROS_INFO("Number of joints in chain %i", n_joints);
                       ...
                       const joint_trajectory_controller::State &state_error)
    {
        ...
        int8_t output_error_code = id_solver->CartToJnt(
                joints_state.q,
                joints_state.qdot,
                outer_loop_control,
                external_forces,
        ...
        if (output_error_code != 0) {
            ROS_ERROR("error solving inverse dynamics, error code %i", output_error_code);
            throw std::runtime_error("error solving inverse dynamics");
        }
...
private:
    ...
    typedef realtime_tools::RealtimePublisher<gripper_control::Efforts> EffortsPublisher;
